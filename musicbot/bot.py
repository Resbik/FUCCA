...
#######################################################################################################################
    ...
    async def cmd_노래(self, message, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
                ...
                try:
                    if 'track' in parts:
                        ...
                        for i in res['tracks']['items']:
                            ...
                            await self.cmd_노래(message, player, channel, author, permissions, leftover_args, song_url)
        ...
        async with self.aiolocks[_func_() + ':' + str(author.id)]:
            ...
            if 'entries' in info:
                ...
                try:
                    ...
                    return await self.cmd_노래(player, channel, author, permissions, leftover_args, e.use_url)
    ...
    async def cmd_생방송(self, player, channel, author, permissions, song_url):
        """
        Usage:
    ...
    async def cmd_노래검색(self, message, player, channel, author, permissions, leftover_args):
            ...
            if not leftover_args:
                ...
                raise exceptions.CommandError(
                    self.str.get('cmd-search-noquery', "Please specify a search query.\n%s") % dedent(
                        self.cmd_노래검색.__doc__.format(command_prefix=self.config.command_prefix)),
                    expire_in=60
                )
        ...
        for e in info['entries']:
            ...
            if str(reaction.emoji) == '\u2705':  # check
                await self.safe_delete_message(result_message)
                await self.cmd_노래(message, player, channel, author, permissions, [], e['webpage_url'])
    ...
    async def cmd_노래시간(self, player, channel, guild, message):
        """
        Usage:
            {command_prefix}np
    ...
    async def cmd_뿌까(self, channel, guild, author, voice_channel):
        """
        Usage:
